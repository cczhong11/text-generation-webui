...
from extensions.多模态模型.abstract_pipeline import AbstractMultimodalPipeline
...
def load_pipeline(params: dict) -> Tuple[AbstractMultimodalPipeline, str]:
    ...
    for name in available_pipeline_modules:
        try:
            pipeline_modules[name] = import_module(
                f"extensions.multimodal.pipelines.{name}.pipelines"
            )
        except:
    ...
    if shared.args.multimodal_pipeline is not None:
        for k in pipeline_modules:
            if hasattr(pipeline_modules[k], "get_pipeline"):
                pipeline = getattr(pipeline_modules[k], "get_pipeline")(
                    shared.args.multimodal_pipeline, params
                )
    ...
    logger.critical(
        f"{log} Please specify a correct pipeline, or disable the extension"
    )
    raise RuntimeError(
        f"{log} Please specify a correct pipeline, or disable the extension"
    )
