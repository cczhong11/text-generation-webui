...
from extensions.多模态模型.multimodal_embedder import MultimodalEmbedder
...
def tokenizer_modifier(state, prompt, input_ids, input_embeds):
    ...
    image_match = re.search(
        r'<img src="data:image/jpeg;base64,[A-Za-z0-9+/=]+">', prompt
    )
...
def ui():
    ...
    with gr.Column():
        ...
        single_image_checkbox = gr.Checkbox(
            False, label="Embed all images, not only the last one"
        )
    # Prepare the input hijack
    picture_select.upload(
        lambda picture: input_hijack.update(
            {"state": True, "value": partial(add_chat_picture, picture)}
        ),
        [picture_select],
        None,
    )
