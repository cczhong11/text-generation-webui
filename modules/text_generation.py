...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    if shared.model_type in ["rwkv", "llamacpp"]:
        ...
        return input_ids
    elif shared.model_type in ["chatglmcpp"]:
        input_ids = shared.tokenizer.encode(str(prompt), 512)
        input_ids = np.array(input_ids).reshape(1, len(input_ids))
        return input_ids
    else:
    ...
    if shared.model_type in ["rwkv", "llamacpp", "chatglmcpp"] or shared.args.cpu:
        return input_ids
...
def generate_reply_wrapper(question, state, eos_token=None, stopping_strings=None):
    for reply in generate_reply(
        question, state, eos_token, stopping_strings, is_chat=False
    ):
...
def _generate_reply(
    question, state, eos_token=None, stopping_strings=None, is_chat=False
):
    ...
    if generate_func is None:
        ...
        if shared.model_type in ["rwkv", "llamacpp", "chatglmcpp"]:
...
def generate_reply_HF(
    ...
    try:
        ...
        new_tokens = len(output) - (
            original_tokens if shared.model_type != "HF_seq2seq" else 0
        )
        print(
...
def generate_reply_custom(
    ...
    if shared.model_type == "llamacpp":
        for k in ["mirostat_mode", "mirostat_tau", "mirostat_eta"]:
            generate_params[k] = state[k]
    if shared.model_type == "chatglmcpp":
        for k in ["do_sample"]:
            generate_params[k] = state[k]
    ...
    try:
        ...
        if not state["stream"]:
            if shared.model_type == "chatglmcpp":
                ...
                reply = shared.model.generate(question, **generate_params)
            if not is_chat:
                reply = apply_extensions("output", reply)
            ...
            for reply in shared.model.generate_with_streaming(
